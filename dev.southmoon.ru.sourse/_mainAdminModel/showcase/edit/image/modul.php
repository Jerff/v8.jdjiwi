	...
	protected function updateIsErrorData($data, &$isError) {
		if(!$this->getId()) {
			if(!$this->getFilter('id')) {
				$isError = true;
				$this->getForm()->setError('name', 'Выберите сначало подраздел');
			} else {
				if(!cmfModulLoad('showcase_edit_db')->getFeildOfId('id', $this->getFilter('id'))) {
					$isError = true;
					$this->getForm()->setError('name', 'Раздел выбрать нельзя');
				}
			}
		}
		return $isError;
