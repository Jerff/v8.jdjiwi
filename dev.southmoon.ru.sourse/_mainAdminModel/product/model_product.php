...
class model_product extends cmfDriverModel {
    ...
    static public function updatePrice($id) {
        self::updateWhere($id);
        cmfRegister::getSql()->placeholder("UPDATE ?t p SET p.isUpdate='yes', p.price=ROUND(IF(p.price1, p.price1, p.price2) * IF(p.type='sale', 1, p.discount)) WHERE ?w", db_product, $id);
    }
    ...
    static public function updateColor($list) {
        ...
        foreach((array)$list as $id) {
            $res = $sql->placeholder("(SELECT color FROM ?t WHERE id=?)
                                      UNION
                                      (SELECT color FROM ?t WHERE product=?)",
                                      db_product, $id, db_product_image, $id)
                                      ->fetchAssocAll();
            $color = array();
            foreach($res as $row) {
                $color = array_merge($color, cmfString::pathToArray($row['color']));
            }
            $sql->placeholder("UPDATE ?t SET colorAll=? WHERE id=?", db_product, cmfString::arrayToPath(array_unique($color)), $id);
        }
    }
    ...
    static private function startDbProductDd() {
        cmfRegister::getSql()->placeholder(" CREATE TEMPORARY TABLE IF NOT EXISTS `db_product_id` (
                                              `id` mediumint(8) unsigned NOT NULL,
                                              PRIMARY KEY  (`id`))");
		cmfRegister::getSql()->placeholder("REPLACE INTO `db_product_id` SELECT id FROM ?t WHERE section IN(SELECT id FROM ?t WHERE isVisible='yes') AND brand IN(SELECT id FROM ?t WHERE visible='yes') AND ?w", db_product, db_section, db_brand, self::isVisible());
	}
    ...
    static public function updateUri($sec=null, $brand=null, $product=null) {
        ...
        cmfRegister::getSql()->placeholder("
                REPLACE ?t SELECT '/product/', sec.id, 0, p.id, CONCAT_WS('/', sec.isUri, p.uri) FROM ?t p LEFT JOIN ?t sec ON(sec.id=p.section) WHERE sec.id=p.section AND ?w:sec AND ?w:p",
                db_content_url, db_product, db_section, $sec, $product);
        /* brand */
        /*cmfRegister::getSql()->placeholder("
		...
		cmfRegister::getSql()->placeholder("
                DELETE FROM ?t WHERE page='/product/' AND product NOT IN(SELECT id FROM `db_product_id`)",
                db_content_url);
	}
    ...
    static public function updateSearch($sec=null, $brand=null, $product=null) {
        ...
        foreach($section as $id=>$path) {
            $parent = array();
            foreach(cmfString::pathToArray($path) as $v) {
                $parent[$v] = $v;
            }
            $parent[$id] = $id;
            $section[$id] = $parent;
        ...
        foreach($res as $row) {
            ...
            $productDump = cmfString::unserialize($row['paramDump']);
            ...
            if($filter) {
                if(isset($_basketId[$sectionId=$row['section']])) {
                    ...
                    $product = $sql->placeholder("SELECT s.basket AS secBasket, s.path AS secPath FROM ?t s WHERE s.id=?", db_section, $sectionId)
                                   ->fetchAssoc();
                    $path = cmfString::pathToArray($product['secPath']);
    			...
    			if(isset($filter['color'])) {
                	$filter['color'] = cmfString::pathToArray($row['color']);
                } else {
	...
	static private function saveProduct($filter, $row, $i=1) {
        if(!$filter) {
            cmfRegister::getSql()->replace(db_search, $row);
			return;
		}
	...
	static public function isProduct() {
		...
		$sql->placeholder("UPDATE ?t s SET s.isNewProduct=(SELECT count(*) FROM ?t p WHERE p.brand=s.brand AND p.id IN(SELECT t.id FROM ?t t WHERE t.created>". (time() - cmfConfig::read('catalog', 'novelty') *24*60*60) ."))
