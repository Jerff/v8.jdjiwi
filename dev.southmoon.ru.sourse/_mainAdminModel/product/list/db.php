...
class product_list_db extends driver_db_list {
	...
	protected function getSort() {
		$section = $this->getSql()->placeholder("SELECT GROUP_CONCAT(id) FROM ?t ORDER BY path, pos", db_section)
		                            ->fetchRow(0);
		if($section) {
	...
	protected function getWhereFilter() {
        ...
        if($articul) {
            $filter[] = $this->getSql()->getQuery("articul LIKE '%?s%'", $articul);
			return $filter;
        }
		...
		$price1 = $this->getFilter('price1');
		$price2 = $this->getFilter('price2');
		if($price1 or $price2) {
			$price1 = cmfToFloat($price1);
			$price2 = cmfToFloat($price2);
		...
		switch($this->getFilter('filter')) {
            ...
            case 'new':
                $filter[] = "`created`>'". (time() - cmfConfig::read('catalog', 'novelty') *24*60*60) ."' AND";
                break;
	...
	public function getProductList($section, $filter=null, $fileds=null) {
		...
		$res = $this->getSql()->placeholder("SELECT ?fields:p FROM ?t p WHERE p.section ?@ AND ?w:p ORDER BY ?o:p", $fileds, $this->getTable(), $section, $filter, $this->getSort())
								->fetchAssocAll('id');
		return $res;
	}
