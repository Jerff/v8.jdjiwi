...
class basket_status_list_db extends driver_db_list {
	...
	public function loadData(&$row) {
		$pay = cmfString::pathToArray($row['pay']);
        $new = array();
	...
	public function getStatusList($isPay=null) {
		$where = $isPay ? array($this->getSql()->getQuery("pay='' OR pay LIKE '%[?s]%'", $isPay)) : array(1);
		return $this->getSql()->placeholder("SELECT id, name, color, stop FROM ?t WHERE ?w ORDER BY stop, pos", $this->getTable(), $where)
								->fetchAssocAll('id');
	}
