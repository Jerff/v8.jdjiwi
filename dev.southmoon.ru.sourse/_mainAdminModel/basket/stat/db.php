...
class basket_stat_db extends driver_db_list {
	...
	protected function getWhereFilter() {
		...
		if($status!=='all') {
			$filter[] = $this->getSql()->getQuery("status IN(SELECT id FROM ?t WHERE stop=?)", db_basket_status, $status);
			$filter[] = 'AND';
		}
		...
		$filter[] = $this->getSql()->getQuery("orderDate>=? AND orderDate<?", date('Y-m-d H:i:s', $start), date('Y-m-d H:i:s', $end));
