...
//		$row['dump'] = get($row, 'dump')==='yes' ? 'есть' : 'нет';
	...
	protected function runListQuery($id=null, $offset=null, $limit=null) {
		...
		return $this->getSql()->placeholder("SELECT SQL_CALC_FOUND_ROWS ?f FROM ?t WHERE id IN(SELECT product2 FROM ?t WHERE article=?) AND ?w ORDER BY ?o LIMIT ?i, ?i ", $this->getFields(), $this->getTable(), db_article_attach, $action, $this->getWhereFilter(), $this->getSort(), $offset, $limit);
	}
