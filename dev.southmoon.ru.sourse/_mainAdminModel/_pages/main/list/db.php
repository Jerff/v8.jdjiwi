...
class _pages_main_list_db extends driver_db_list_one {
	...
	public function getSelectTree(&$element, $index=false, $isTitle=true) {
		...
		if($isTitle) {
			$_title=array();
			$res = $sql->placeholder("SELECT uri,CONCAT(IF(LENGTH(title),'t',0),IF(LENGTH(keywords),'k',0),IF(LENGTH(description),'d',0)) FROM ?t", db_seo_title);
			while($row=$res->fetchRow())
				$_title[$row[0]] = '['. $row[1] .'] ';
			$res->free();
		}

		$res = $sql->placeholder("SELECT c.id, c.parent, c.name, c.small_name  FROM ?t c WHERE c.parent ?@ AND c.type='pages' AND c.url!='' AND c.visible='yes' ORDER BY c.pos", $this->getTable(), $id);
		$pages = array();
