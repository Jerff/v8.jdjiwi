...
abstract class driver_modul_gallery_edit extends driver_modul_edit {
    ...
    public function updatePreview() {
	    ...
	    $main = cmfWWW . $this->getGalleryPath() . $data['image_main'];
        ...
        $image = cmfBaseImg . $this->getGalleryPath() . $data[$this->getGalleryId()] .'?'. time();
        $this->getResponse()->script("$('#galleryPreviewId img, #preview". $this->getId() ."').attr('src', '{$image}');");
	}
	...
	protected function updateThumbnail($res, $small, $width, $height) {
	    ...
	    $small = cmfWWW . $this->getGalleryPath() . $small;
