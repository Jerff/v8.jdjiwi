...
abstract class driver_db_edit_param extends driver_db_edit {
    ...
    public function deleteProduct($id) {
		$this->getSql()->del(db_param_select, array('id'=>$id));
		$this->getSql()->del(db_param_checkbox, array('id'=>$id));
	}
	...
	public function saveElement($param, $value) {
		...
		if(!$_param) {
		    ...
		    list($data, $price, $dump) = $this->getSql()->placeholder("SELECT param, paramPrice, paramDump, count FROM ?t WHERE id=?", $this->getTable(), $this->getId())
		                            ->fetchRow();
            $data = cmfString::unserialize($data);
            $price = cmfString::unserialize($price);
            $dump = cmfString::unserialize($dump);
		}
		...
		switch($type=get2($_param, $id, 'type')) {
			...
			case 'radio':
				if($value<0 or is_null($value)) {
					unset($data[$param]);
					$this->getSql()->del(db_param_select, array('id'=>$this->getId(), 'AND', 'param'=>$param));
				} else {
					$data[$param] = $value;
					$this->getSql()->replace(db_param_select, array('id'=>$this->getId(), 'param'=>$param, 'value'=>$value));
				}
				break;
			...
			case 'checkbox':
				...
				if($value==='yes') {
					$data[$param][$key] = $key;
					$this->getSql()->replace(db_param_checkbox, array('id'=>$this->getId(), 'param'=>$param, 'value'=>$key));
				} else {
					unset($data[$param][$key]);
					$this->getSql()->del(db_param_checkbox, array('id'=>$this->getId(), 'AND', 'param'=>$param, 'AND', 'value'=>$key));
				}
				break;
		...
		$this->setUpdateData($this->getId(), $param);
	}
