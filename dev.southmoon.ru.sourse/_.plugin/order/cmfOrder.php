...
class cmfOrder {
    ...
    static public function initPrice($_product, $_basket=null, $pricePay=null) {
        ...
        $_colorAll = cmfRegister::getSql()->placeholder("SELECT id, name, color FROM ?t WHERE visible='yes' ORDER BY name", db_color)
                                    ->fetchAssocAll('id');
        $res = cmfRegister::getSql()->placeholder("SELECT p.id, IF(p.type!='sale' AND p.discount='1', '1', '0') AS isDiscount,
        ...
        foreach($res as $id=>$row) {
            ...
            $productPrice = cmfString::unserialize($row['paramPrice']);
            ...
            $color = cmfString::pathToArray($row['color']);
    ...
    static public function newOrder($data) {
        ...
        return cmfRegister::getSql()->add(db_basket, $data);
    }


	static public function &getPayTypeList() {
		if(false===($res = cmfCache::get('cmfOrder::getPayTypeList'))) {
			$res = cmfRegister::getSql()->placeholder("SELECT id, name FROM ?t ORDER BY pos", db_basket_pay)
											->fetchRowAll(0, 1);
	...
	static public function &getKeyStatusList() {
		...
		if(false===($res = cmfCache::getParam('cmfOrder::getKeyStatusList', $stop))) {
			...
			$res = cmfRegister::getSql()->placeholder("SELECT id FROM ?t WHERE stop ?@", db_basket_status, $stop)
											->fetchRowAll(0, 0);
	...
	static public function &getStatusList() {
		...
		if(false===($res = cmfCache::getParam('cmfOrder::getStatusList', $stop))) {
			...
			$res = cmfRegister::getSql()->placeholder("SELECT id, name FROM ?t WHERE stop ?@", db_basket_status, $stop)
											->fetchRowAll(0, 1);
	...
	static public function getStartStatus() {
        return cmfRegister::getSql()->placeholder("SELECT id FROM ?t WHERE `stop`='1' ORDER BY pos LIMIT 0, 1", db_basket_status)
									->fetchRow(0);
	}
