...
<request>
    ...
    static public function success() {
        if(isset($_POST['operation_xml'])) {
            ...
            switch((string)$xml->status) {
                case 'success':     cmfGlobal::set('paymentMessage', 'Платеж прошел успешно'); break;
                case 'failure':     cmfGlobal::set('paymentMessage', 'Платеж не прошел');; break;
                case 'wait_secure': cmfGlobal::set('paymentMessage', 'Платеж в обработке'); break;
            }
    ...
    static public function view($data) {
        ...
        $desc = cmfString::translate($data['desc']);
