...
class cmfPaymentDriver {
    ...
    static public function addLog() {
        ...
        return cmfRegister::getSql()->add(db_payment_log, array('type'=>$type,
    ...
    static public function isTransactions($type, $transaction) {
        $is = cmfRegister::getSql()->placeholder("SELECT 1 FROM ?t WHERE `type`=? AND `transaction`=?", db_payment_transactions, $type, $transaction)
                                    ->numRows();
        if($is) exit;
    }
    ...
    static public function resultOk($type, $orderId, $transaction, $sum, $data) {
        ...
        $userData['adminUrl'] = cmfProjectAdmin .'#/basket/edit/&id='. $orderId;
        $userData['orderUrl'] = cmfGetUrl('/user/order/one/', array($orderId));
		...
		$payData = $type.'<br />'. print_r(cmfString::objectToArray($data), true);
