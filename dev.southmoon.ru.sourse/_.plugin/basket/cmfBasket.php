...
cmfLoad('order/cmfOrder');
cmfLoad('order/cmfDiscount');
class cmfBasket {
	...
	function __construct($isLoad = true) {
		...
		$product = cmfCookie::get(self::name .'Product');
		if(!$product) return;
		$this->filterData($product);
		$this->step = cmfSession::get(self::name .'Step');

		$this->count = cmfCookie::get(self::name .'Count');
		$this->price = cmfCookie::get(self::name .'Price');
	}


	private function filterData($data) {
		foreach(cmfString::unserialize($data) as $productId=>$list) {
	...
	protected function saveUser() {
		cmfRegister::getUser()->updateBasket($this->product ? serialize($this->product) : '');
	}
	...
	public function save($isSaveUser=true) {
		$this->setCokies();
		if(cmfAjax::is()) {
			cmfAjax::get()->script('cmf.basket.header();');
		}
		if($isSaveUser) {
    		$this->saveUser();
        }
	...
	public function setCokies() {
		cmfCookie::set(self::name .'Product', serialize($this->product), 12);
		cmfSession::set(self::name .'Step', $this->step);
		cmfCookie::set(self::name .'Count', $this->count, 12);
		cmfCookie::set(self::name .'Price', cmfPrice::format($this->price), 12);
	}
	public function disable() {
		cmfCookie::del(self::name .'Product');
		cmfSession::del(self::name .'Step');
		cmfCookie::del(self::name .'Count');
		cmfCookie::del(self::name .'Price');
