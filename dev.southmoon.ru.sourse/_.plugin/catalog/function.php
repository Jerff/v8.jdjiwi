...
cmfLoad('catalog/cmfPrice');
cmfLoad('catalog/cmfParam');
cmfLoad('catalog/cmfProductUrl');
cmfLoad('catalog/sort');
cmfLoad('catalog/filter');
function &cmfGetNoSection() {
	if(false===($no = cmfCache::get('cmfGetNoSection'))) {

		list($no1) = cmfRegister::getSql()->placeholder("SELECT GROUP_CONCAT(id) FROM ?t WHERE isVisible='no'", db_section)
									->fetchRow();
		list($no2) = cmfRegister::getSql()->placeholder("SELECT GROUP_CONCAT(id) FROM ?t WHERE visible='no'", db_brand)
									->fetchRow();
        $no = array($no1, $no2);
