...
class cmfCronProductDump extends cmfCronUpdateDriver {
    ...
    static public function update() {
        ...
        foreach($res as $k=>$v) {
            $_parser[cmfString::strtolower($k)] = $v;
        ...
        $res = $sql->placeholder("SELECT id, value FROM ?t WHERE id ?@", db_param, $_param)
                   ->fetchAssocAll();
        foreach($res as $row) {
            $_param[$row['id']] = cmfString::unserialize($row['value']);
        }
        ...
        foreach($res as $row) {
            ...
            $url = cmfGetAdminUrl('/admin/product/edit/') .'id='. $row['id'];
            ...
            foreach($tmp2[1] as $value) {
                $value = cmfString::strtolower(trim(strip_tags($value)));
            ...
            $isUpdate = true;
            $sql->add(db_product, array('param'=>cmfString::serialize($param)), $row['id']);
    ...
    static private function message($message, $important=false) {
        cmfRegister::getSql()->add(db_product_dump_log, array('date'=>date('Y-m-d H:i:s'),
                                                          'content'=>$message,
