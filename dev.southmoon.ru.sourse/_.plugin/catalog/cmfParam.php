...
class cmfParam {
	...
	static public function getNotice($group, $basket=0) {
        if(!$res = cmfCache::getParam('cmfParam::getNotice', $group)) {
            if(!$basket) {
                $basket = cmfRegister::getSql()->placeholder("SELECT `basket` FROM ?t WHERE id ?@ AND basket!=0 ORDER BY path DESC LIMIT 0, 1", db_section, $group)
                                                                            ->fetchRow(0);
            }
	...
	static public function getPrice($path) {
        if(!$res = cmfGlobal::getParam('cmfParam::getPrice', $path)) {
            if(!$res = cmfCache::getParam('cmfParam::getPrice', $path)) {
                $res = cmfRegister::getSql()->placeholder("SELECT name, value FROM ?t WHERE id=(SELECT `basket` FROM ?t WHERE id ?@ AND basket!=0 ORDER BY path DESC LIMIT 0, 1)", db_param, db_section, cmfString::pathToArray($path))
                                                                                ->fetchRow();
                if($res[1]) $res[1] = unserialize($res[1]);
                cmfCache::setParam('cmfParam::getPrice', $path, $res, 'sectionList,paramList');
            }
            cmfGlobal::setParam('cmfParam::getPrice', $path, $res);
        }
        return $res;
    }
