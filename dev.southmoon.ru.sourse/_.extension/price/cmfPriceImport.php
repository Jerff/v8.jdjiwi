...
class cmfPriceImport {
    ...
    protected function saveLog($shop) {
        if($this->_log)
        cmfRegister::getSql()->add(db_shop_import, array('shop'=>$shop,
    ...
    public function __construct($shop, $file) {
        ...
        cmfLoad('excel_reader2/excel_reader2');
        ...
        for($i=2; ; $i++) {
            ...
            if(count($row)>1) {
                ...
                $send['color'] = cmfString::arrayToPath($new);
    ...
    protected function getSection($section, $name) {
        $res = cmfRegister::getSql()->placeholder("SELECT id, basket FROM ?t WHERE parent=? AND name=?", db_section, $section, $name)
                    ->fetchRow();
        if($res) {
