...
class cmfPriceExport {
    ...
    public function __construct($shop, $uri) {
        ...
        cmfLoad('Spreadsheet/Excel/Writer');
	...
	protected function param() {
        ...
        foreach($param as $id=>$row) {
            $param[$id]['value'] = cmfString::unserialize($row['value']);
        }
	...
	protected function exportProduct($section, $basket) {
        ...
        foreach($res as $id=>$row) {
            $color = array();
            foreach(cmfString::pathToArray($row['color']) as $v) {
                $color[] = get($_color, $v);
            }
            $paramPrice = cmfString::unserialize($row['paramPrice']);
