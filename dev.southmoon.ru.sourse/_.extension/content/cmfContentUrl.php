...
class cmfContentUrl {
	...
	static public function updateShowcase() {
 	...
 	public static function isUrlExists($page, $id, $uri, $brand=0, $product=0) {
        $url = '/'. $uri . '/';
        return cmfRegister::getSql()->placeholder("SELECT 1 FROM ?t WHERE `part`='main' AND (`small_name`=? OR `url`=?)", db_pages_main, $url, $url)
									->numRows()
        		or cmfRegister::getSql()->placeholder("SELECT 1 FROM ?t WHERE `url`=? AND `url` NOT IN(SELECT `url` FROM ?t WHERE `id`=? AND `page`=? AND `brand`=? AND `product`=?)", db_content_url, $uri, db_content_url, $id, $page, $brand, $product)
									    ->numRows();
	}
