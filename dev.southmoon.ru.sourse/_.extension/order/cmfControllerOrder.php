...
class cmfControllerOrder {
    ...
    static public function updatePrice($orderId, $productList, $newList, $deleteList) {
        ...
        $paramDump = cmfRegister::getSql()->placeholder("SELECT p.id, p.count, p.paramDump FROM ?t p WHERE p.id ?@ AND p.isOrder='no'", db_product, array_merge(array_keys($_basket), array_keys($_basketNew)))
                                          ->fetchAssocAll('id');
        foreach($paramDump as $id=>$row) {
            $dump = cmfString::unserialize($row['paramDump']);
            $count = $row['count'];
            $isEdit = false;
...
//                pre($id, $pId, $cValue, get3($_basketNew, $id, 'count', $pId, array()));
            ...
            if($isEdit) {
                cmfRegister::getSql()->add(db_product, array('count'=>$count, 'paramDump'=>cmfString::serialize($dump)), $id);
            }
        }
        $send = array();
    ...
    static public function setType($id, $status) {
        ...
        $send['orderUrl'] = cmfGetUrl('/user/order/one/', array($id));


        cmfSmsInform::changeStatus($status, $send);

        $send = array('status'=>$status, 'changeDate'=>date('Y-m-d H:i:s'), 'adminId'=>cmfRegister::getAdminId());
		if($stop1==1) {
            switch($stop2) {
                case 0:
    ...
    static public function changePay($id) {
        if(self::addOrderPayment($id)) {
            cmfRegister::getSql()->add(db_basket, array('isPay'=>'yes'), $id);
        }
    }

    static public function addOrderPayment($id, $type=null, $transaction=0) {
        $row = cmfRegister::getSql()->placeholder("SELECT user, pay FROM ?t WHERE id=? AND isPay='no'", db_basket, $id)
                                    ->fetchAssoc();
        if($row) {
            $send = array('id'=>$id,
                          'user'=>$row['user'],
            ...
            if($transaction) {
                $is = cmfRegister::getSql()->placeholder("SELECT 1 FROM ?t WHERE id=? AND `type`=? AND transaction=?", db_basket_order, $id, $type, $transaction)
                                        ->numRows();
            } else {
                $is = cmfRegister::getSql()->placeholder("SELECT 1 FROM ?t WHERE id=? AND `user`=?", db_basket_order, $id, $row['user'])
                                        ->numRows();
            }
            if($is) {
                cmfRegister::getSql()->placeholder("UPDATE ?t SET `isOk`='yes' WHERE id=?", db_basket_order, $id);
            } else {
                cmfRegister::getSql()->replace(db_basket_order, $send);
            }
            cmfDiscount::update();
            return true;
        } else {
    ...
    static public function returnProductDump($list) {
        $_basket = cmfRegister::getSql()->placeholder("SELECT p.id, p.product FROM ?t p WHERE p.id ?@", db_basket, (array)$list)
                                    ->fetchRowAll(0, 1);
        foreach($_basket as $product) {
            list(, $_basket) = cmfString::unserialize($product);
            $paramDump = cmfRegister::getSql()->placeholder("SELECT p.id, p.count, p.paramDump FROM ?t p WHERE p.id ?@ AND p.isOrder='no'", db_product, array_keys($_basket))
                                    ->fetchAssocAll('id');
            foreach($_basket as $id=>$row) if(isset($paramDump[$id])) {
                $dump = cmfString::unserialize($paramDump[$id]['paramDump']);
    ...
    static public function setStatusIsOk($list) {
        cmfRegister::getSql()->placeholder("UPDATE ?t SET `isOk`='yes' WHERE id ?@", db_basket_order, (array)$list);
        cmfDiscount::update();
    }

    static public function setStatusIsNoOk($list) {
        self::returnProductDump($list);
        cmfRegister::getSql()->placeholder("UPDATE ?t SET `isOk`='no' WHERE id ?@", db_basket_order, (array)$list);
        cmfDiscount::update();
    }

    static public function delete($list) {
        self::returnProductDump($list);
        cmfRegister::getSql()->placeholder("UPDATE ?t SET `delete`='yes' WHERE id ?@", db_basket, (array)$list);
        cmfRegister::getSql()->placeholder("UPDATE ?t SET `delete`='yes' WHERE id ?@", db_basket_order, (array)$list);
        cmfDiscount::update();
    }
