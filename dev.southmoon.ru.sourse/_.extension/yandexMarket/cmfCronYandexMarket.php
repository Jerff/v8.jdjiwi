...
class cmfCronYandexMarket extends cmfCronUpdateDriver {
    ...
    static public function run() {
        $conf = cmfConfig::read('yandex.market');
        $date = date('Y-m-d H:i');
        $index = cmfProjectMain;
...
HTML;
    ...
    static public function getSection() {
        $res = cmfRegister::getSql()->placeholder("SELECT id, parent, path, basket, name, stop FROM ?t WHERE id IN(SELECT section FROM ?t WHERE brand='0' AND isMenu='yes') AND isVisible='yes' ORDER BY `path`, pos", db_section, db_section_is_brand)
                                    ->fetchAssocAll();
        $_section = $_sectionPath = $_path = $_pathId = array();
        foreach($res as $row) {
            $tmp = cmfString::pathToArray($row['path']);
            $tmp[] = $row['id'];
            ...
            $_section[$row['id']] = array(  'parent'=>$row['parent'],
                                            ...
                                            'name'=>cmfString::specialchars($row['name']));
        }

        $name = cmfRegister::getSql()->placeholder("SELECT id, name FROM ?t WHERE id ?@", db_section, $_pathId)
                                    ->fetchAssocAll('id');
    ...
    static public function getProduct($file, $conf, $_section, $_sectionPath) {
        ...
        foreach($res as $k=>$row) {
            ...
            switch($isNotice) {
                ...
                case 'param+content':
                case 'content':
                    $description = cmfHtmlEntityDecode(strip_tags($row['notice']));
                    break;
            }
            ...
            $name = cmfString::specialchars($name);
            $brand = cmfString::specialchars($_brand[$row['brand']]);
            $url = cmfString::specialchars(cmfGetUrl('/product/', array($row['url'])));
            $description = cmfString::specialchars($description);
            $salesNotes = cmfString::specialchars($row['salesNotes']);
            $dump = $row['url']==='yes' ? 'true' : 'false';
            $image = $row['image'] ? cmfString::specialchars(cmfBaseImg . cmfPathProduct . $row['image']) : null;
...
HTML;
            ...
            if($param) {
                foreach($param as $k=>$v) {
                    $html .="
            <param name=\"". cmfString::specialchars($k) ."\">". cmfString::specialchars($v) ."</param>";
                }
            }
