...
class cmfMail {
	...
	public function loadConfig($config=null) {
		if(!$config) {
			$config = cmfRegister::getSql()->placeholder("SELECT login, secure, password, host, port FROM ?t WHERE id='1'", db_mail_config)
											->fetchAssoc();
		}
	...
	private function searchAttachment(&$message) {
        ...
        if(isset($tmp[2])) {
            foreach($tmp[2] as $img) {
                ...
                $this->AddAttachment(realpath(cmfWWW . $img), $file);
            }
        }
	}
	...
	public function sendTemplates($name, $data, $email) {
		list($header, $content, $html) = cmfRegister::getSql()->placeholder("SELECT header, content, html FROM ?t WHERE name=?", db_mail_templates, $name)
														->fetchRow();
		if(!$header) $header = $name;

		$data['proectUrl'] = cmfProjectMain;
