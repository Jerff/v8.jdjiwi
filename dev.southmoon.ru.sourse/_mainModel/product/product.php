...
$productId = cmfGlobal::get('$productId');
...
if($product = cmfCache::getParam('product', array($sectionId, $brandId, $productId))) {
    ...
    $productParam = cmfString::unserialize($product['param']);
    $productPrice = cmfString::unserialize($product['paramPrice']);
    $productDump = cmfString::unserialize($product['paramDump']);
    if($product['colorAll']) {
    	$productParam['color'] = cmfString::pathToArray($product['colorAll']);
    }
    ...
    foreach($res as $v) {
        if(!$imageMain) {
            $imageMain = cmfBaseImg . cmfPathProduct . $v['main'];
            $imageProduct = cmfBaseImg . cmfPathProduct . $v['product'];
        }
        $image[$v['id']] = array('title'=>cmfString::specialchars($v['name'] ? $v['name'] : $product['name']),
                	             'color'=>$v['color'],
                                 'main'=>cmfBaseImg . cmfPathProduct . $v['main'],
                                 'product'=>cmfBaseImg . cmfPathProduct . $v['product'],
                                 'small'=>cmfBaseImg . cmfPathProduct . $v['small']);
    }
    ...
    $this->assing2('title', cmfString::specialchars($product['name']));
    $this->assing('productId', $productId);

    $this->assing('feedback',        $feedback);
    $this->assing('feedbackForm',    $feedback->getForm());
    $this->assing('feedbackContent', $feedbackContent);

    cmfGlobal::set('$searchUri', $product['secUrl']);
    cmfMenu::setSelect('$rootId', $product['secRoot'] ? $product['secRoot'] : $sectionId);
    cmfGlobal::set('$rootId', $product['secRoot'] ? $product['secRoot'] : $sectionId);
    cmfGlobal::set('$sectionList', $path);
    ...
    foreach($_menu as $row) {
        cmfMenu::add($row['name'], cmfGetUrl('/section/', array($row['isUri'])));
    }
    if($brandId) {
        cmfMenu::add($product['bName'], cmfGetUrl('/section/', array($sectionUri .'/'. $product['bUri'])));
    }
    ...
    $this->assing2('sizeUrl', cmfGetUrl('/info/size/'));
    $this->assing('basket', $basket);
    $this->assing2('basketName', mb_strtolower($basket['name'], cmfCharset));
