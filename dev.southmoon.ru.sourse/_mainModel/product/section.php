...
$itemUri = cmfPages::getParam(5);
$nameUri = cmfPages::getParam(7);
$paramUri = cmfPages::getParam(9);
$typeUri = cmfPages::getParam(11);
$sortUri = cmfPages::getParam(13);
$page = (int)cmfPages::getParam(15);
if(!$page) $page = 1;
$limit = cmfPages::getParam(17);
$_itemUri = array();
...
if(isset($_GET['form'])) {
    if(!empty($_POST['searchName'])) {
        ...
        if($name!=='Поиск...' and $name!=='Артикул...') {
            $_itemUri['name'] = $name;
            cmfRedirect(cmfGetUrl('/section/search/', cmfProductUrl::generate($_itemUri)));
        }
    }
    if(empty($itemUri)) {
        cmfRedirect(cmfGetUrl('/search/'));
    } else {
        ...
        cmfRedirect(cmfGetUrl('/section/', cmfProductUrl::generate($_itemUri)));
    }
}
...
$isSearch = cmfPages::isMain('/section/search/') && (!empty($nameUri) || !empty($itemUri));
if($isSearch) {
	if(empty($nameUri)) {
	...
	cmfGlobal::set('$searchName', htmlspecialchars(urldecode($nameUri)));
...
cmfGlobal::set('$_itemUri', $_itemUri);
...
cmfGlobal::set('$isSale', $isSale);
cmfGlobal::set('$isNew', $isNew);

$_limit = explode('-', cmfConfig::get('catalog', 'productList'));
$_limit = array_combine($_limit, $_limit);
$limitStart = cmfConfig::get('catalog', 'productLimit');
...
if($isSearch) {
    ...
    if(!empty($_itemUri['section'])) {
        $row = cmfRegister::getSql()->placeholder("SELECT page, id AS section, brand, product FROM ?t WHERE url=?", db_content_url, $_itemUri['section'])
                                ->fetchRow();
        if(!$row) return 404;
...
if($sectionId) {
    if($section = cmfCache::getParam('section', $sectionId)) {
        ...
        $section = $sql->placeholder("SELECT id, parent, root, path, name, isUri, title, keywords, description FROM ?t WHERE id=? AND visible='yes'", db_section, $sectionId)
        				->fetchAssoc();
        $path = cmfString::pathToArray($section['path']);
        $menu = array();
        if($path) {
        ...
        $_article = array();
        $res = $sql->placeholder("SELECT id, header, date, image, image_title, notice, uri FROM ?t WHERE section=? AND visible='yes' ORDER BY isMain, date DESC LIMIT 0, ?i", db_article, $sectionId/*array_merge($sectionList, $path)*/, cmfConfig::get('article', 'articleSection'))
    							->fetchAssocAll();
        foreach($res as $row) {
    ...
    foreach($menu as $k=>$v) {
        if(isset($v['uri'])) {
            cmfMenu::add($v['name'], cmfGetUrl($sectionDefault, array($v['uri'])));;
        }
    }

    if($brandId or $isSale or $isNew or count($_itemUri)>1) {
        cmfMenu::add($section['name'], cmfGetUrl($sectionDefault, array($section['isUri'])));
    } else {
    ...
    cmfGlobal::set('$secPath', $path);
    cmfGlobal::set('$sectionName', $section['name']);
    cmfGlobal::set('$searchUri', $sectionUri);

    cmfMenu::setSelect('$rootId', $section['root'] ? $section['root'] : $sectionId);
    cmfGlobal::set('$rootId', $section['root'] ? $section['root'] : $sectionId);
    cmfGlobal::set('$sectionUri', $section['isUri']);
    cmfGlobal::set('$sectionList', $sectionList);
...
if($brandId) {
    ...
    cmfGlobal::set('$brandUri', $brand['uri']);

    if($isNew or $isSale) {
        cmfMenu::add($brand['name'], $sectionId ? cmfGetUrl('/brand/section/', array($section['isUri'], $brand['uri'])) :
                                                  cmfGetUrl('/brand/', array($brand['uri'])));
    } else {
...
if($isNew) {
    $where[] = 'AND';
    $where[] = "`created`>'". (time() - cmfConfig::get('catalog', 'novelty') *24*60*60) ."'";
    cmfMenu::add('Новинки');
} else if($isSale) {
    $where[] = 'AND';
    $where['type'] = 'sale';
...
if($search = cmfCache::getParam('section', array($sectionId, $brandId, $searchId))) {
    ...
    if($paramFilter) {
	    ...
	    foreach($paramList as $k=>$row) {
	        if($k==='price') {
                ...
                $step = ceil(($max-$min)/cmfConfig::get('param', 'priceStep')/500)*500;
	            $row['value'][0] = "Цена";
	            if($min+$step<$max) {
    ...
    $_discount = $sql->placeholder("SELECT name, CONCAT(?, image) FROM ?t WHERE visible='yes'", cmfPathDiscount, db_param_discount)
                   ->fetchRowAll(0, 1);
...
if($product) {
    ...
    foreach($res as $row) {
        $_product[$row['id']] = array('name'=>$row['name'],
                                      ...
                                      'isOrder'=>$row['isOrder'],
                                      'image'=>cmfBaseImg . cmfPathProduct . $row['image_section'],
    ...
    if($_product) {
        ...
        $_page_url = cmfPagination::generate($page, $count, $limit, cmfConfig::get('catalog', 'productPage'),
...
foreach($paramList as $k=>$v) {
	...
	foreach($v['value'] as $k2=>$v2) {
        $paramFilter[$k]['value'][$k2] = array('name'=>$v2,
                                               ...
                                               'url'=>cmfGetUrl($sectionPage, cmfProductUrl::replace($_uri, 'param', $id, $k2, 'page', 1)));
	}
	$id++;
}
...
foreach(array('asc'=>'По возрастанию цены',
    ...
    $list[$k] = array('name'=>$v,
                      'url'=>cmfGetUrl($sectionPage, cmfProductUrl::replace($_itemUri, 'sort', $k, 'page', 1)));
}
...
foreach($_limit as $v) {
    $list[$v] = array('name'=>$v,
                      'url'=>cmfGetUrl($sectionPage, cmfProductUrl::replace($_itemUri, 'limit', $limitStart==$v ? null : $v)));
}
    $list['all'] = array('name'=>'Все',
                         'url'=>cmfGetUrl($sectionPage, cmfProductUrl::replace($_itemUri, 'limit', 'all', 'page', 1)));
if(isset($list[$limit])) {
	$list[$limit]['sel'] = true;
} elseif(isset($isAll)) {
    $list['all']['sel'] = true;
...
$this->assing2('searchUrl', $url = cmfGetUrl($sectionDefault, cmfProductUrl::replace($_itemUri, 'page', 1)));
$this->assing2('defaultName', 'Артикул...');
$this->assing2('searchName', cmfGlobal::get('$searchName', 'Артикул...'));
cmfGlobal::set('$searchUrl', $url)
